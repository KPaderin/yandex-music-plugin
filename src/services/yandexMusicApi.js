export class YandexMusicApi {
    ...
    authorization(token) {
        const url = `/yandex/https://api.music.yandex.net/account/status`
    ...
    getPlaylists() {
        const url = `/yandex/https://api.music.yandex.net/users/${this.userId}/playlists/list`
        return this.request(url, this.headers)
    }

    search(req, type = 'track', nocorrect = false) {
        const url = `/yandex/https://api.music.yandex.net/search?text=${req}&page=0&type=${type}&nocorrect=${nocorrect}&perPage=1`
        return this.request(url, this.headers)
    ...
    createPlaylists(playlistName, visibility = 'private') {
        const url = `/yandex/https://api.music.yandex.net/users/${this.userId}/playlists/create`
    ...
    addTrackById(tracks= [{"id":'72790205',"albumId":'12562338'}], kind = '1004', revision = 8) {
        const url = `/yandex/https://api.music.yandex.net/users/${this.userId}/playlists/${kind}/change-relative`
